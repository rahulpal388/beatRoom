...
const browseList = [
    {
        title: "New Release",
        link: "/dashboard/new-release"
    },
    {
        title: "Top Playlist",
        link: "/dashboard/top-playlist"
    },
    {
        title: "Top Album",
        link: "/dashboard/top-album"
    },
    {
        title: "Top Artists",
        link: "/dashboard/top-artists"
    }
]
...
export default function Browse() {
    ...
    useEffect(() => {
        const fetchDetail = async () => {
            const [newReleaseResponse, topPlaylistResponse, album] = await Promise.all([
    ...
    return <>
        <div className=" py-6 px-8 max-sm:px-4  ">
            ...
            <div className="relative lg:hidden h-screen overflow-y-scroll ">
                <button className=" w-full h-10  px-4 flex  justify-between items-center rounded-sm border-[1.2px]  hover:border-primary border-netural-200 cursor-pointer "
                    onClick={() => { setIsDropDowm(!isDropDown) }}
                >
                    <p>Browse</p>
                            ...
                            isDropDown && (
                                ...
                                >
                                    <ul className="flex flex-col">
                                        {browseList.map(list => (
                                            <Link href={list.link} className=" hover:bg-white px-4 h-8 flex items-center ">
                                                <li>{list.title}</li>
                                            </Link>
                                        ))}
                                    </ul>
